<template>
  <div>
    <van-nav-bar :title="'与'+titles+'聊天中'" left-text="返回" :fixed="true" left-arrow @click-left="onClickLeft" />
    <div class="neirong">
      <ul>
    ...
    <van-row type="flex" justify="space-between">
      ...
      <van-col span="18">
          <input type="text" id="txtinput" v-model="chating" placeholder="等待输入" @keydown.enter="sendinfo" style="width:90%;"/>
      </van-col>
      <van-col span="3">
...
export default {
  ...
  data() {
    return {
      shows:false,
      chating:"",
      actiondata:[{iconInfo:"相册",icons:"photo-o"},
  ...
  methods: {
    ...
    sendinfo(){
        this.chatdata.push({chatinfo:this.chating, username: "self",headimg:this.$store.state.selfInfo.headportrait})
        axios({
          url:"http://i.itpk.cn/api.php",
          params:{question:this.chating,
                  api_key:"d59d916609b385a065e8cb316deb17ec",
                  api_secret:"0a83lrp1kb16",
          },
        }).then((data)=>{
          console.log(data.data)
        this.chatdata.push({chatinfo:data.data, username: "other",headimg:this.otherinfo.headportrait})
        this.chating = "";
